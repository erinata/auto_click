...
module AutoClickMethods
  ...
  def key_stroke(*keys)
    down_keys = keys.map do |key| code = VirtualKey.code_from_name(key)
    InputStructure.keyboard_input(code, 0x0000)
    end
    up_keys   = keys.map do |key| code = VirtualKey.code_from_name(key)
    InputStructure.keyboard_input(code, 0x0002)
    end
    send_input(down_keys + up_keys)
  end
  ...
  def type(string)
    ...
    string = string.to_s
    string.each_char do |c| if ('a'..'z').include? c
      key_stroke(c.to_sym)
    elsif ('A'..'Z').include? c
      key_down(:leftshift)
      key_stroke(c.to_sym)
